...
class OptPair_MaxSharpe(BaseOpt):
    ...
    def __init__(self, config_file_yaml, curr_date, run_pairs
        ...
        if cache_namespace is None:
            cache_namespace = 'CtaYmjh_'.join(run_pairs)
        ...
        # 策略对象缓存
        namespace = '_'.join(['CtaYmjhStrategy'] + list(self._run_pairs))
        if not cache.load(namespace=namespace):
            self._strategy_obj = CtaYmjhStrategy_ex(config=self._config, curr_date=self._curr_date, symbol_list=self._run_pairs,
    ...
    def get_dailyreturns(self, volLookback, s_period, l_period, re_calc = False):
        ...
        if not re_calc:
            daily_return_by_init_aum = cache.get_object((volLookback, s_period, l_period),
                                                        namespace=self._cache_namespace)  # self.get_cache(volLookback,speeds,scaler)
        ...
        if daily_return_by_init_aum is None:  # 缓存无结果
            ...
            _config.strategy_config['s_period'] = s_period * 2
            _config.strategy_config['l_period'] = s_period * 2 + l_period * 2
            ...
            if settlement_obj is not None:
                ...
                daily_return_by_init_aum = daily_return_by_init_aum[self._strategy_obj.start_date:]
    ...
    def objective(self, trial):
        ...
        s_period = trial.suggest_int('s_period', 2, 15)
        l_period = trial.suggest_int('l_period', 1, 20)
