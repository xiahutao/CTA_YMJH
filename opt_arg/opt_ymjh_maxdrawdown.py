...
from ctaymjh1 import CtaYmjhStrategy_ex
...
class OptPair_MaxSharpe(BaseOpt):
    ...
    def __init__(self, config_file_yaml, curr_date, run_pairs, look_back_start, look_back_end
        ...
        _strategy_obj = cache.get_object('CtaYmjhStrategy', namespace='_strategy_obj')
        if _strategy_obj is not None:
            self._strategy_obj = _strategy_obj
        else:
            self._strategy_obj = CtaYmjhStrategy_ex(config=self._config, curr_date=curr_date, symbol_list=run_pairs,
                                                asset_type=global_variable.ASSETTYPE_FUTURE)
            self._strategy_obj.load_data(startDate=None, endDate=None)
            cache.set_object('CtaYmjhStrategy', self._strategy_obj, namespace='_strategy_obj')
    ...
    def objective(self, trial):
        ...
        '''
        # 优化的自变量
        volLookback = trial.suggest_int('volLookback', 0, 6)  # 在20，120 之间的（整数）离散区间优化
        s_period = trial.suggest_int('s_period', 3, 30)
        l_period = trial.suggest_int('l_period', 1, 40)

        # 查询缓存相同参数下的daily_return结果
        daily_return = self.get_cache(volLookback, s_period, l_period)
        if daily_return is None:  # 缓存无结果
            print('back test', 20*volLookback, s_period, s_period + l_period)

        # 执行回测
            _config = copy.copy(self._config)
            _config.strategy_config['volLookback'] = volLookback * 20
            _config.strategy_config['s_period'] = s_period
            _config.strategy_config['l_period'] = l_period + s_period
...
class OptPair_MinMaxDrawDown(OptPair_MaxSharpe):
    ...
    def objective(self, trial):
        ...
        if isinstance(daily_return, pandas.Series):
            ...
            # 返回目标值（当前为回测的最大回撤值）
            return Analysis_func.max_drawdown(daily_returns=daily_return_)
        # 返回目标值（当前为回测的夏普值）
        return -1
...
def run(each_pair0, config_file_yaml0):
    ...
    cache.set_object('CtaYmjhStrategy', strategy_obj, namespace='_strategy_obj')
    ...
    print(opt_date_list)
    best_arg_path = r'E:/Strategy/YMJH/best_args_MaxDrawDown'
    ...
    for look_back_end in opt_date_list:
        ...
        # n_trials，
        testopt1 = OptPair_MinMaxDrawDown(curr_date=look_back_end.strftime('%Y-%m-%d'), look_back_start=look_back_start,
