...
if __name__ == "__main__":
    ...
    chatPath = 'e://Strategy//MOMENTUM//fig//'
    ...
    #             else:
    #                 result_folder = 'e://Strategy//YMJH//resRepo_ymjh_%s_%s_%s' % (clas, s_period, l_period)
    #                 evalue = pd.read_csv(result_folder + '//evaluation.csv', index_col=0)
    #                 harvest_row.append(1 * evalue[state_name].tolist()[0])
    #                 # evalue['s_period'] = s_period
    #                 # evalue['l_period'] = l_period
    #                 # evalue['class'] = clas
    #                 # state.append(evalue[state_name].tolist()[0])
    ...
    #     plt.show()
    #     # 参数组合下的sharpe的分布图
    s_period_lst = [i for i in range(129, 2, -6)]
    l_period_lst = [i for i in range(6, 262, 12)]
