...
from data_engine.setting import ASSETTYPE_FUTURE
from config.config import Config_back_test
from backtest_config_cta_ymjh.ctaymjh import CtaYmjhStrategy_ex,get_day_and_night_symbols,get_trade_days
...
def gen_config(product, config_file_yaml, curr_date, log_file=None, config_file=r'e://Strategy//MOMENTUM//configs',
    ...
    try:
        ...
        outputDir = config.strategy_config['config_file_output']
        for each in run_symbols:
            ...
            strategy_obj = CtaYmjhStrategy_ex(config=config, curr_date=curr_date,
            ...
            if not config_ret._config_object :
                continue
            # 保存配置信息
...
if __name__ == '__main__':
    ...
    for config_file_yaml in ['config_sample_1d.yaml']:
        ...
        try:
            log.debug('apply_async: symbols' + '_'.join(run_symbols))
        ...
        config = Config_back_test(config_path=os.path.split(__file__)[0]).load(config_file=config_file_yaml)
        tmp_obj = CtaYmjhStrategy_ex(curr_date=datetime.datetime.now(), config=config,
                                         asset_type=global_variable.ASSETTYPE_FUTURE, symbol='RB_VOL')
        mail_to_list = ['519518384@qq.com']
